...
class GetActiveBrandsController extends Controller
{
    ...
    // }
    public function get_active_brands(Request $request)
    {
        // $this->authorize('view-any', Brand::class);

        $main_char_id = env('BRAND_ID');
        ...
        if ($request->has('colors') && count($request->brands) > 0) {
        ...
        $query = $query->select('CLMCHARVALUEID as id', 'CLMCHARVALUE_ML_NAME as name', 'CLMCHARVALUE_PICTURE as image')
                ->orderBy('CLMCHARVALUEPRIORITY', 'desc')
                ->orderBy('CLMCHARVALUE_ML_NAME', 'asc')
                ->paginate($perPage, ['/*'], 'page', $page);

        foreach ($query as $key=>$row) {
            $query[$key]->link = env('APP_URL').'/page/search?brands='.$row->id;
            $query[$key]->checked = false;
            if(in_array($row->id, $brands_array)){
